		...
		return this.state.challenges.map((challenge, index) => {
			return (
				<tr key={challenge._id}>
					{" "}
					<td>{index + 1}</td> <td>{challenge.title}</td> <td>{challenge.description}</td>{" "}
					<td>
						<Link className={"text-warning"} to={`/challenge/${challenge._id}`}>
							{" "}
							<SiJavascript />
						</Link>
					</td>{" "}
			...
			<div>
				...
				<Container className={"mt-5"}>
					{this.props.user.isCompany ? (
						<Link to='/challenges/add' className={"btn btn-outline-success text-center mb-3"}>
							+ Add Challenge
						</Link>
					) : (
						""
