...
class ChallengesDetails extends Component {
    ...
    getOutput() {
        ...
        let returnVal = ''
        switch (typeof this.state.challenge.input) {
            case 'string':  parameterValue = `"${this.state.challenge.input}"`
                break
            case 'number': parameterValue = `${this.state.challenge.input}`
                break
        }
        switch (typeof this.state.challenge.output) {
            case 'string':  returnVal = `"${this.state.challenge.output}"`
                break
            case 'number': returnVal = `${this.state.challenge.output}`
                break
        }
        ...
        return (
            <div>
                ...
                <Container className={'mt-5'}>
                    <h1 className={'text-success'}>{this.state.challenge.title}</h1>
                    <h5 className={''}><span className="text-success font-weight-bold">Description:</span> {this.state.challenge.description}</h5>
                    <Row>
                        <Col xs={6}>
                            <p className="text-success font-weight-bold">Input:</p>
                    <CodeMirror
                        value={this.getInput()}
                        options={{
                            mode: "javascript",
                    ...
                    />
                        ...
                        <Col xs={6}>
                            <p className="text-success font-weight-bold">Output:</p>
                    <CodeMirror
